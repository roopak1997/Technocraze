...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"athena\")"
  ...
  echo
  echo "	byfn.sh generate -c athena"
  echo "	byfn.sh up -c athena -s couchdb"
  echo "        byfn.sh up -c athena -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c athena"
  echo "        byfn.sh upgrade -c athena"
  echo
...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.clinicaltrials.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.clinicaltrials.*/) {print $3}')
...
CLI_DELAY=3
# channel name defaults to "athena"
CHANNEL_NAME="athena"
